...
/**
 * This page shows the list of users and provides the ability to search for a user.
 * Provides a link to a page with all of that user's questions, answers, reviews (if applicable),
 * and messages (if applicable).
 */
public class AdministrationSearchPage {
	...
	/**
	 * Constructs a new page for a given staff member.
	 * 
	 * @param db The application's DatabaseHelper instance.
	 * @param user The current user of the application.
	 */
    public AdministrationSearchPage(DatabaseHelper db, User user) {
    	this.db = db;
        this.user = user;
        pageTitle = "Moderation Home Page";
    }
    
    /**
     * Shows the AdministrationSearchPage on the given stage.
     * <p>
    ...
    public void show(Stage primaryStage) {
	    ...
	    searchText.setPrefWidth(500.0);
	    Button searchButton = new Button("Execute");
	    searchBox.getChildren().addAll(searchText, searchButton);
        ...
        refreshButton.setOnAction(e -> {
        	new AdministrationSearchPage(db, user).show(primaryStage);
        });
        ...
        searchButton.setOnAction(e -> {
        	String searchedUser = searchText.getText();
        	if (UserNameRecognizer.checkForValidUserName(searchedUser).equals("") 
        		...
        		DialogReturns retVal = dialogPg.show(DialogTypes.twoButtonYesNo,
        				"Open User?", "Do you want to open the moderation panel with this user?");
        		
        		if (retVal == DialogReturns.yes) {
        			/* Open mod pane */
        			new ModerationForUserPage(db, user, searchedUser).show();
        		}
        	}
        	else {
        ...
        layout.getChildren().addAll(topBox, searchBox, errorLabel, scroller);
    ...
    private void addToList(String listUser, VBox content) {
        ...
        userName.setOnAction(a -> {
        	new ModerationForUserPage(db, user, listUser).show();
        });
